...
while True:
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            # Debug markers
            i = lm[8].y * h < lm[6].y * h - 15
            m = lm[12].y * h < lm[10].y * h - 15
            r = lm[16].y * h < lm[14].y * h - 15
            p = lm[20].y * h > lm[18].y * h + 10
            t = abs(lm[4].x - lm[2].x) < 0.05
            debug_text = f"I:{i} M:{m} R:{r} P:{p} T:{t}"
            cv2.putText(frame, debug_text, (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
    ...
    cv2.imshow("Touchless Interface ALPHA", frame)
